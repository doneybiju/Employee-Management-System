...
export default function CreateUserAuto() {
  ...
  const isAllowed = !!user && (user.role === 'hr' || user.role === 'super_admin');
  ...
  if (!isLoggedIn) return <div><Link href="/login">Login</Link> required.</div>;
  ...
  (async () => {
    ...
    try {
      ...
      const norm: Dept[] = Array.isArray(raw)
        ? raw.map((d: any) => ({
            id: Number(d.id),
            departmentName: d.departmentName ?? d.name ?? d.department_name ?? '',
  ...
  const deptName = departments.find(x => x.id === Number(selectedDeptId))?.departmentName;
...
const finalEmail =
  ...
  const onDeptChange = (val: string) => {
    if (!val) { setSelectedDeptId(''); setSelectedPosId(''); return; }
...
async function postProvisionWithFallback(payload: any) {
  ...
  for (const p of paths) {
    try {
      ...
      if (msg.includes('Cannot GET') || msg.includes('Cannot POST') || msg.includes('Not Found') || msg.includes('404')) {
        lastErr = e;
        continue;
      }
      throw e;
  ...
  const canSubmit =
    firstName && surname && personalEmail && selectedDeptId && selectedPosId && joiningDate && gender && nationality;
    ...
    <div className={s.wrap}>
      <header className={s.head}>
        ...
        <div className={s.emailPreview}>
          ...
          <div className={s.emailNote}>Final email and EMP ID are generated on submit to ensure uniqueness.</div>
        </div>
      </header>

      <div className={s.formWrap}>
        {notice && <div className={`${s.notice} ${notice.kind === 'success' ? s.success : s.error}`}>{notice.msg}</div>}

        <form onSubmit={onSubmit}>
          <div className={s.grid}>
            <div className="form-group">
              <label className={`${s.required}`}>First name</label>
              <input className={s.input} required value={firstName} onChange={(e) => setFirstName(e.target.value)} />
            </div>

            <div className="form-group">
              <label className={s.required}>Surname</label>
              <input className={s.input} required value={surname} onChange={(e) => setSurname(e.target.value)} />
            </div>

            <div className={`${s.full}`}>
              <label className={s.required}>Personal email</label>
              <input className={s.input} required type="email" value={personalEmail} onChange={(e) => setPersonalEmail(e.target.value)} />
            </div>

            <div className="form-group">
              <label className={s.required}>Nationality</label>
              <CountrySelect label="" value={nationality} onChange={setNationality} />
            </div>

            <div className="form-group">
              <label className={s.required}>Gender</label>
              <select className={s.select} required value={gender} onChange={(e) => setGender(e.target.value)}>
            ...
            <div className="form-group">
              ...
              <input className={s.input} type="date" value={birthdate} onChange={(e) => setBirthdate(e.target.value)} />
            </div>

            <div className="form-group">
              <label className={s.required}>Department</label>
              <select className={s.select} required value={selectedDeptId} onChange={(e) => onDeptChange(e.target.value)}>
                <option value="">{loadingDepts ? 'Loading…' : deptError ? 'Failed to load' : '-- Select Department --'}</option>
            ...
            <div className="form-group">
              ...
              >
                <option value="">
                  {!selectedDeptId ? 'Select a department first' : positionsForDept.length ? '-- Select Position --' : 'No positions'}
                </option>
                {positionsForDept.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
              </select>
            </div>

            <div className="form-group">
              <label className={s.required}>Start date</label>
              <input className={s.input} required type="date" value={joiningDate} onChange={(e) => setJoiningDate(e.target.value)} />
            </div>
  ...
  >
    ...
    <option value="team_lead">Team Lead</option>
            ...
            <div className="form-group">
              ...
              <input className={s.input} value={supervisor} onChange={(e) => setSupervisor(e.target.value)} />
            </div>


          
            <div className={`${s.full}`}>
              <button type="submit" className={s.submitBtn} disabled={submitting || !canSubmit}>
        ...
        {result && (
          <div className={s.resultBox}>
            ...
            {result.companyEmail && result.google && result.google.created === false && (
              <div className={s.resultItem}>
                <button
                  type="button"
            ...
            {result.tempPasswords && (
              <>
                <div className={s.resultItem}>
                  ...
                  <div className={s.code}>{result.tempPasswords.google ?? '—'}</div>
                </div>
                <div className={s.resultItem}>
                  <div className={s.resultLabel}>Site temp password:</div>
                  <div className={s.code}>{result.tempPasswords.site ?? '—'}</div>
                </div>
              </>
            )}
          </div>
