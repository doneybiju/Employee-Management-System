...
type ProjectTaskLite = { status: 'NOT_STARTED' | 'IN_PROGRESS' | 'BLOCKED' | 'COMPLETED' };
...
const STATUS_OPTIONS = ['NOT_STARTED','IN_PROGRESS','ON_HOLD','COMPLETED'] as const;
...
setProjects(normalized);
  ...
  const loadMemberCandidates = async () => {
    try {
      const response = await fetchWithAuth('/api/projects/member-candidates?q=');
      if (response.ok) {
        const data = await response.json();
...
async function updateProjectStatus(projectId: number, newStatus: ProjectStatus) {
  try {
  ...
  const getStatusCount = (status: string) => {
    return projects.filter(project => status === 'ALL' || project.status === status).length;
  };
    ...
    <div className={styles.container}>
      ...
      <div className={styles.statsOverview}>
        ...
        <div className={styles.statCard}>
          <div className={styles.statValue}>
            {totalProjects ? Math.round((completedProjects / totalProjects) * 100) : 0}%
          </div>
      ...
      <section className={styles.filterSection}>
        ...
        <div className={styles.statusTabs}>
          {(['ALL', 'NOT_STARTED', 'IN_PROGRESS', 'COMPLETED', 'ON_HOLD'] as const).map((statusKey) => (
            <button
              key={statusKey}
              className={`${styles.statusTab} ${statusFilter === statusKey ? styles.active : ''}`}
              onClick={() => setStatusFilter(statusKey)}
              style={statusFilter === statusKey ? {
            ...
            >
              ...
              {statusKey.replace('_', ' ')}
              <span className={styles.statusCount}>{getStatusCount(statusKey)}</span>
            </button>
          ))}
        </div>
      </section>
...
<div className={styles.formGroup}>
  ...
  <select
    ...
    onChange={(e) =>
      setEditingProject(p => p ? { ...p, status: e.target.value as ProjectStatus } : p)
    }
    ...
    {STATUS_OPTIONS.map(s => (
      <option key={s} value={s}>{s.replace('_',' ')}</option>
        ...
        <div className={styles.modalActions}>
          ...
          <button type="submit" disabled={savingEdit || !editTitle.trim()} className={styles.submitBtn}>
