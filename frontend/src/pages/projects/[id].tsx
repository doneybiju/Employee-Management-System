...
function computeProjectStatus(tasks: { status: Task['status'] }[]):
...
export default function ProjectDetailPage() {
  ...
  const isAuthorized = !!user && (user.role === 'super_admin' || user?.empType === 'team_lead');
...
if (!dead) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetchWithAuth(`/api/projects/member-candidates?q=${encodeURIComponent(search)}`, { signal: ctrl.signal } as any);
        if (!res.ok) return;
...
async function removeMember(userId: number, memberName: string) {
  ...
  const assigned = (proj.tasks || []).filter(t => t.assignedTo?.id === userId);
  if (assigned.length) {
  ...
  try {
    const res = await fetchWithAuth(`/api/projects/${projId}/members/${userId}`, { method: 'DELETE' });
    if (!res.ok) {
      const j = await res.json().catch(() => ({}));
      if (res.status === 409 && j?.error === 'user_has_assigned_tasks' && Array.isArray(j?.tasks)) {
  ...
  async function createTask(e: FormEvent) {
    ...
    try {
      ...
      // reset form
      setTTitle(''); setTDesc(''); setTDue(''); setTAssign('');
      setTChecklistEnabled(false); setTChecklistItems(['']);
  ...
  async function deleteTask(taskId: number) {
    ...
    try {
      const res = await fetchWithAuth(`/api/projects/tasks/${taskId}`, { method: 'DELETE' });
      if (!res.ok) {
        const j = await res.json().catch(() => ({}));
        throw new Error(j?.error || `${res.status} ${res.statusText}`);
      }
  ...
  async function updateTask(taskId: number, patch: Partial<{
    title: string; description: string|null; dueDate: string|null;
    assigneeId: number|null; status: Task['status']; checklistEnabled: boolean;
  }>) {
  ...
  async function setChecklistEnabled(taskId: number, enabled: boolean) {
    const r = await fetchWithAuth(`/api/projects/tasks/${taskId}/checklist-enabled`, {
      method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ checklistEnabled: enabled }),
    });
  ...
  async function toggleChecklistItem(taskId: number, itemId: number, done: boolean) {
    const r = await fetchWithAuth(`/api/projects/tasks/${taskId}/checklist/${itemId}`, {
  ...
  async function removeChecklistItem(taskId: number, itemId: number) {
    const r = await fetchWithAuth(`/api/projects/tasks/${taskId}/checklist/${itemId}`, { method:'DELETE' });
...
const [editingTaskId, setEditingTaskId] = useState<number | null>(null);
  ...
  function TaskCard({
  t, members, canEdit, expanded, onToggle,
  isEditing, onStartEdit, onCancelEdit,canAct, 
}: {
  t: Task; members: Member[]; canEdit: boolean;
  ...
  isEditing: boolean; onStartEdit: () => void; onCancelEdit: () => void;
   canAct: boolean;
    ...
    const [eDue, setEDue] = useState(t.dueDate ? new Date(t.dueDate).toISOString().slice(0,10) : '');
...
const checklistProgress = t.checklistEnabled && t.checklistItems.length > 0
  ? Math.round((t.checklistItems.filter(item => item.done).length / t.checklistItems.length) * 100)
  : 0;


    return (
      <div className={`${styles.taskCard} ${expanded ? styles.expanded : ''}`}>
        <div className={styles.taskHeader} onClick={() => !isEditing && onToggle()}>
  ...
  />
              ...
              <div className={styles.taskStatusBadge} style={{ 
            ...
            <div className={styles.taskPreviewMeta}>
              ...
              {t.dueDate && (
                <span className={`${styles.dueDatePreview} ${new Date(t.dueDate) < new Date() ? styles.overdue : ''}`}>
...
>
              ...
              <>
                ...
                <div className={styles.taskDetailsGrid}>
                  ...
                  <div className={styles.detailItem}>
                    ...
                    <span className={`${styles.detailValue} ${t.dueDate && new Date(t.dueDate) < new Date() ? styles.overdue : ''}`}>
                    ...
                    <div className={styles.checklistTitle}>
                      ...
                      {t.checklistEnabled && t.checklistItems.length > 0 && (
                        <div className={styles.checklistProgress}>
                          ...
                          <span className={styles.progressText}>{checklistProgress}%</span>
                        </div>
                      )}
                    </div>
                    {canEdit && (
                      <button 
                        ...
                        onClick={() => setChecklistEnabled(t.id, !t.checklistEnabled)}
                      >
  ...
  type="checkbox"
  checked={ci.done}
  onChange={(e) => canAct && toggleChecklistItem(t.id, ci.id, e.target.checked)}
  disabled={!canAct}
  className={styles.checklistCheckbox}
/>

                              <span className={`${styles.checklistText} ${ci.done ? styles.completed : ''}`}>
                                {ci.title}
                              </span>
                            </label>
                            {canEdit && (
                              <button 
                                ...
                                onClick={() => removeChecklistItem(t.id, ci.id)}
                                title="Delete item"
                              >
                                ×
                              </button>
  ...
  <div className={styles.statusActionsSection}>
    ...
    <div className={styles.statusActionsGrid}>
      {(['NOT_STARTED','IN_PROGRESS','BLOCKED','COMPLETED'] as const).map(s => (
        ...
        >
          <span className={styles.statusIcon}>{statusIcons[s]}</span>
          <span className={styles.statusText}>{s.replace('_', ' ')}</span>
          {t.status === s && <span className={styles.currentIndicator}>✓</span>}
        </button>
      ))}
    </div>
  </div>
              ...
              <div className={styles.taskEditForm}>
                ...
                <div className={styles.formRow}>
                  <div className={styles.formGroup}>
                    ...
                    <select 
                      value={eAssign} 
                      onChange={e => setEAssign(e.target.value ? Number(e.target.value) : '')}
...
const projectProgress = computeProgress(proj.tasks);
  ...
  <Link href="/projects" className={styles.backButton}>
    ...
    <div style={{ width:140, height:8, background:'#eee', borderRadius:4, overflow:'hidden' }}>
      <div style={{ height:'100%', width:`${projectProgress}%`, background:'#4cc9f0' }} />
    </div>
    <span style={{ fontSize:12, color:'#555' }}>{projectProgress}%</span>
        ...
        <p className={styles.projectDescription}>{proj.description || 'No description provided'}</p>
        
        <div className={styles.projectMeta}>
          <div className={styles.metaInfo}>
            <span className={styles.metaLabel}>Due Date:</span>
            <span className={styles.metaValue}>
              {proj.dueDate ? new Date(proj.dueDate).toLocaleDateString() : 'No due date'}
            </span>
          </div>
          <div className={styles.metaInfo}>
      ...
      <div className={styles.projectContent}>
        ...
        <section className={styles.teamSection}>
          ...
          <div className={styles.membersGrid}>
            {proj.members.length ? proj.members.map(m => (
      ...
      ✖ Remove
        ...
        <section className={styles.tasksSection}>
          ...
          <div className={styles.tasksGrid}>
            {proj.tasks.length ? proj.tasks.map(t => (
              <TaskCard
  key={t.id}
  t={t}
...
/>
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modal}>
            <div className={styles.modalHeader}>
              ...
              <button className={styles.modalClose} onClick={() => setShowMemberModal(false)}>×</button>
            </div>
            ...
            <form onSubmit={addMembers} className={styles.modalForm}>
                    ...
                    return (
                      <label key={c.id} className={styles.candidateItem}>
                        <input
                          ...
                          checked={checked}
                          onChange={() => {
                            setSelectedIds(s => checked ? s.filter(x => x !== c.id) : [...s, c.id]);
                          }}
                          className={styles.candidateCheckbox}
                        />
                        <div className={styles.candidateInfo}>
                          <div className={styles.candidateName}>{c.name}</div>
                          {c.email && <div className={styles.candidateEmail}>{c.email}</div>}
                        </div>
                      </label>
                    );
              ...
              <div className={styles.modalActions}>
                ...
                >
                  {addingMembers ? 'Adding...' : `Add ${selectedIds.length} Member(s)`}
                </button>
              </div>
            </form>
          </div>
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modal}>
            <div className={styles.modalHeader}>
              ...
              <button className={styles.modalClose} onClick={() => setShowTaskModal(false)}>×</button>
            </div>
            ...
            <form onSubmit={createTask} className={styles.modalForm}>
              ...
              <div className={styles.formRow}>
                <div className={styles.formGroup}>
                  ...
                  <select 
                    value={tAssign} 
                    onChange={e => setTAssign(e.target.value ? Number(e.target.value) : '')}
                    className={styles.formSelect}
                  >
                    <option value="">Unassigned</option>
                    {proj.members.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
                  </select>
                </div>
  ...
  onClick={() => {
    const next = tChecklistItems.filter((_, i) => i !== idx);
