...
function ImportUsersPageInner() {
  ...
  const [companyEmailMode, setCompanyEmailMode] = useState<CompanyEmailMode>('csv');

const isCompanyEmailField = (f: ImportField) => {
  const id = (f.id || '').toLowerCase();
  const label = (f.label || '').toLowerCase();
  return (id.includes('company') && id.includes('email')) || (label.includes('company') && label.includes('email'));
};
  ...
  { id: 'map', label: 'Map Columns', description: 'Match CSV columns to fields' },
  { id: 'result', label: 'Import Result', description: 'Review import outcome' },
];

const stepIndex = steps.findIndex((s) => s.id === step);
const progressPct = steps.length <= 1 ? 0 : (stepIndex / (steps.length - 1)) * 100;
...
const retryFailedEmails = async () => {
  ...
  try {
    ...
    setResult((prev) => {
      ...
      return {
        ...prev,
        setupEmails: mergeByUserId(prev.setupEmails || [], resp.results || []),
        setupEmailsSent: (mergeByUserId(prev.setupEmails || [], resp.results || []).filter((r) => r.ok).length),
        setupEmailsFailed: (mergeByUserId(prev.setupEmails || [], resp.results || []).filter((r) => !r.ok).length),
      };
    });
  } finally {
  ...
  const getUsedFieldIdsExcludingColumn = (excludeColIndex: number): Set<string> => {
  ...
  return fields.filter((f) => {
    if (companyEmailMode === 'generate' && isCompanyEmailField(f)) return false;
    return f.required && (mapping[f.id] == null || Number.isNaN(Number(mapping[f.id]!)));
  });
};
...
const res = await commitEmployeeImport(file, mappingToSend, { companyEmailMode });
      setResult(res);
      setStep('result');
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } catch (err: any) {
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '2rem 1.5rem' }}>
      <div style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.875rem', fontWeight: 700, marginBottom: '0.5rem', color: '#1f2937' }}>
          Import Employees
        </h1>
      ...
      <div style={{ marginBottom: '3rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'relative' }}>
          {steps.map((stepItem, index) => {
            ...
            return (
              <div key={stepItem.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 2, flex: 1 }}>
                <div
                  style={{
                    width: '2.5rem',
                    height: '2.5rem',
                    ...
                    fontSize: '0.875rem',
                    marginBottom: '0.5rem',
                    ...(status === 'completed' ? { backgroundColor: '#10b981', color: 'white' } :
                         status === 'current' ? { backgroundColor: '#3b82f6', color: 'white' } :
          ...
          <div style={{ textAlign: 'center', maxWidth: '500px', margin: '0 auto' }}>
            ...
            >
              ...
              <div style={{ marginBottom: '0.5rem' }}>
                <span style={{ color: '#3b82f6', fontWeight: 600 }}>Click to upload</span>
              ...
              {loading && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                  <div style={{ width: '1rem', height: '1rem', border: '2px solid #e5e7eb', borderTop: '2px solid #3b82f6', borderRadius: '50%', animation: 'spin 1s linear infinite' }} />
                  <span style={{ color: '#6b7280', fontSize: '0.875rem' }}>Processing file...</span>
                </div>
              )}
            </div>
            ...
            <div style={{ marginTop: '2rem', textAlign: 'left' }}>
              <h3 style={{ fontWeight: 600, marginBottom: '0.5rem', color: '#374151' }}>CSV Format Requirements</h3>
              <ul style={{ color: '#6b7280', fontSize: '0.875rem', listStyle: 'disc', paddingLeft: '1.5rem' }}>
...
>
  ...
  <div style={{ marginTop: '0.5rem', fontSize: '0.875rem', color: '#6b7280' }}>
    If “Generate new” is selected, the Company Email column will not be imported from the CSV.
  </div>
</div>
      )}
        ...
        >
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            <p style={{ color: '#6b7280', fontSize: '0.875rem' }}>
              Match your CSV columns to the system fields. Required fields are marked with <span style={{ color: '#ef4444' }}>*</span>.
            </p>
          </div>
  ...
  const usedElsewhere = getUsedFieldIdsExcludingColumn(colIndex);
  ...
  return (
    ...
    >
      <div style={{ marginBottom: '0.5rem' }}>
        {h || <span style={{ color: '#9ca3af', fontStyle: 'italic' }}>(empty header)</span>}
      </div>
      ...
      >
        ...
        {preview.fields
          ...
          .filter((f) => !(companyEmailMode === 'generate' && isCompanyEmailField(f)))
          // NEW: hide fields already selected in other columns (but keep current selection)
          .filter((f) => !usedElsewhere.has(f.id) || f.id === currentFieldId)
          .map((f) => (
                  ...
                  >
                    ...
                    {preview.header.map((_h, colIndex) => (
                      <td
                        ...
                        title={row[colIndex] || ''}
                      >
                        {row[colIndex] || <span style={{ color: '#9ca3af' }}>-</span>}
                      </td>
                    ))}
                  </tr>
                ))}
          ...
          <div style={{ 
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
              ...
              <span style={{ fontWeight: 600, color: '#92400e' }}>Required Fields</span>
            </div>
            <div style={{ color: '#92400e', fontSize: '0.875rem' }}>
              {preview.fields
                .filter((f) => f.required && !(companyEmailMode === 'generate' && isCompanyEmailField(f)))
                .map((f) => f.label)
                .join(', ')}
          ...
          <div style={{ marginTop: '2rem', display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            ...
            >
              {loading ? (
                <>
                  <div style={{ width: '1rem', height: '1rem', border: '2px solid transparent', borderTop: '2px solid currentColor', borderRadius: '50%', animation: 'spin 1s linear infinite' }} />
                  Importing...
                </>
              ) : (
                <>
          ...
          <div style={{ textAlign: 'center', maxWidth: '600px', margin: '0 auto' }}>
            ...
            <div style={{ 
              ...
              <div style={{ textAlign: 'center', padding: '1.5rem', backgroundColor: '#f8fafc', borderRadius: '8px' }}>
                ...
                <div style={{ fontSize: '0.875rem', color: '#6b7280' }}>Users Created</div>
              </div>
              <div style={{ textAlign: 'center', padding: '1.5rem', backgroundColor: '#f8fafc', borderRadius: '8px' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#10b981', marginBottom: '0.25rem' }}>
                  {result.createdInterns}
                </div>
                <div style={{ fontSize: '0.875rem', color: '#6b7280' }}>Intern Profiles</div>
              </div>
              <div style={{ textAlign: 'center', padding: '1.5rem', backgroundColor: '#f8fafc', borderRadius: '8px' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#10b981', marginBottom: '0.25rem' }}>
                  {result.createdInternships}
                </div>
                <div style={{ fontSize: '0.875rem', color: '#6b7280' }}>Internships</div>
              </div>
            </div>
            ...
            {(result.skippedExisting > 0 || result.skippedEmpty > 0) && (
              <div style={{ 
                ...
                <h3 style={{ fontWeight: 600, marginBottom: '0.5rem', color: '#92400e' }}>Skipped Rows</h3>
                <div style={{ color: '#92400e', fontSize: '0.875rem' }}>
                  {result.skippedExisting > 0 && <div>• {result.skippedExisting} rows with existing users</div>}
                  {result.skippedEmpty > 0 && <div>• {result.skippedEmpty} empty rows</div>}
                </div>
              </div>
            )}
            ...
            {result.setupEmails && (
  <div style={{ background:'#f8fafc', border:'1px solid #e5e7eb', borderRadius: 10, padding: 16, marginBottom: 16, textAlign:'left' }}>
    <h3 style={{ margin: 0, marginBottom: 8, fontWeight: 600 }}>Password setup emails</h3>

    <div style={{ display:'flex', gap: 16, flexWrap:'wrap', marginBottom: 10 }}>
      <div><strong>Sent:</strong> {result.setupEmailsSent ?? result.setupEmails.filter(r => r.ok).length}</div>
      <div><strong>Failed:</strong> {result.setupEmailsFailed ?? result.setupEmails.filter(r => !r.ok).length}</div>
    </div>
    ...
    <div style={{ maxHeight: 160, overflowY: 'auto', fontSize: 13, color: '#374151' }}>
      {result.setupEmails.map((r) => (
        <div key={r.userId} style={{ padding: '6px 0', borderTop: '1px solid #e5e7eb' }}>
          <div><strong>{r.email}</strong> — {r.ok ? 'Sent' : 'Failed'}</div>
          {!r.ok && r.error && <div style={{ color:'#b91c1c' }}>{r.error}</div>}
        </div>
      ))}
    </div>
  </div>
            ...
            {result.rowErrors.length > 0 && (
              <div style={{ 
                ...
                <h3 style={{ fontWeight: 600, marginBottom: '0.5rem', color: '#dc2626' }}>Import Errors</h3>
                <div style={{ fontSize: '0.875rem', color: '#dc2626', maxHeight: '200px', overflowY: 'auto' }}>
