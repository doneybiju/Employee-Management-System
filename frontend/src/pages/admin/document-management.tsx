...
type DocFilter = 'all' | 'missing_any' | 'acceptance_letter' | 'learning_agreement' | 'passport_id' | 'cv';
...
const kindLabel = (kind: Kind, empType?: EmpType | null) => {
  switch (kind) {
    ...
    case 'learning_agreement': return agreementLabelForEmpType(empType);
    case 'passport_id': return 'Passport ID';
...
const DOC_KEYS: Array<{ kind: Kind; getter: keyof AdminDocs; icon: string }> = [
  { kind: 'acceptance_letter',   getter: 'acceptanceLetter',  icon: 'fa-file-contract' },
  { kind: 'learning_agreement',  getter: 'learningAgreement', icon: 'fa-handshake' },
  { kind: 'passport_id',         getter: 'passportId',        icon: 'fa-passport' },
  { kind: 'cv',                  getter: 'cv',                icon: 'fa-file-alt' },
];

// Map UI kinds → backend enum
const DOC_TYPE_MAP: Record<Kind,'ACCEPTANCE_LETTER'|'LEARNING_AGREEMENT'|'ID_PASSPORT'|'CV'> = {
...
function driveIdFromUrl(u?: string | null) {
  if (!u) return null;
  const m1 = u.match(/[?&]id=([^&]+)/); if (m1) return decodeURIComponent(m1[1]);
  const m2 = u.match(/\/file\/d\/([^/]+)/); if (m2) return m2[1];
  const m3 = u.match(/\/api\/uploads\/drive\/file\/([^/?]+)/); if (m3) return m3[1];
  return null;
}
...
export default function AdminDocumentManagement() {
  ...
  const [docSummary, setDocSummary] = useState<
    Record<string, { acc: boolean; la: boolean; pid: boolean; cv: boolean; missingAny: boolean; }>
  >({});
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const res = await fetchWithAuth(`/api/admin/documents/expiring-passports?months=${expiryMonths}`);
        const j = await res.json();
...
const upRes = await fetchWithAuth(`/api/uploads/drive/document/${selectedKind}`, {
  method: 'POST',
  body: fd as any,
});
...
if (!upRes.ok) throw new Error(up?.error || `HTTP ${upRes.status}`);
      ...
      await fetchWithAuth(`/api/profile/admin/documents/${sel.employeeId}/${selectedKind}/complete`, {
        ...
        body: JSON.stringify({
          url: up?.url, fileId: up?.fileId, originalName: up?.originalName,
          mimeType: up?.mimeType, fileSize: up?.fileSize
        }),
      });
  ...
  const filtered = useMemo(() => {
    ...
    if (searchText.trim()) {
      ...
      rows = rows.filter(u => String(u[searchField] || '').toLowerCase().includes(q));
    }
    if (docFilter !== 'all') {
      rows = rows.filter(u => {
  ...
  const hasAnyDocs =
    !!(docs?.acceptanceLetter || docs?.learningAgreement || docs?.passportId || docs?.cv);
  ...
  <div className="tabs">
    {[
      ...
      {k:'learning_agreement', label: agreementLabelForEmpType(sel?.empType)},
  ...
  <section className="expiry-panel">
    <div className="expiry-panel-head">
      <div>
        ...
        <div className="expiry-subtitle">Shows interns with ID_PASSPORT expiring within the selected range.</div>
      </div>

      <button className="tab" onClick={() => setShowExpiryPanel(false)}>Close</button>
    </div>
    ...
    <div className="expiry-body">
      {expiryLoading ? (
        ...
        <div className="expiry-muted">No passport IDs expiring in this range.</div>
      ) : (
    ...
    >
      ...
      />
                  ...
                  <div className="user-details">
                    ...
                    <div className="user-role">{[u.position, u.department].filter(Boolean).join(' · ') || 'Intern'}</div>
                  </div>
                </div>
              );
            })}
            {!loading && !filtered.length && <div style={{ padding: 12, color: '#7f8c8d' }}>No interns found.</div>}
          </div>
        </aside>
        ...
        <section className="document-management">
          <div className="selected-user">
            ...
            <div className="user-info-text">
              ...
              <p>{sel ? ([sel.position, sel.department].filter(Boolean).join(' · ') || 'Intern') : 'Pick a user from the left'}</p>
            </div>
            {/* Delete avatar */}
            ...
            <div className="upload-card">
              ...
              <input id="file-input" className="file-input" type="file"
                ...
                onChange={e => {
                  ...
                  if (!isPdf) { alert('Only PDF files are allowed.'); e.currentTarget.value=''; return; }
                  setFile(f);
                }} />
            ...
            <div className="document-type">
              {DOC_KEYS.map(d => {
                ...
                return (
                  ...
                  >
                    <div className="doc-icon"><i className={`fas ${d.icon}`} /></div>
...
? (sel?.empType === 'intern' ? 'Internship agreement terms' : 'Employment agreement terms')

                          : 'Official acceptance document'}
                      </p>
                      <div className="doc-status">
                        <span className={`status-indicator ${uploaded ? 'status-uploaded' : 'status-pending'}`}>
                          <i className={`fas ${uploaded ? 'fa-check-circle' : 'fa-clock'}`} />{' '}
    ...
    <i className="fas fa-download" />
      ...
      <style jsx>{`
        *{box-sizing:border-box}
        ...
        .header{display:flex;justify-content:space-between;align-items:center;margin-bottom:30px;padding-bottom:20px;border-bottom:1px solid #e1e4e8}
        .header-title{font-size:28px;color:#2c3e50;font-weight:600}
        .user-info{display:flex;align-items:center;gap:10px;background:#fff;padding:10px 15px;border-radius:8px;box-shadow:0 2px 5px rgba(0,0,0,.05)}
        .user-avatar{width:40px;height:40px;border-radius:50%;object-fit:cover}
        .main-content{display:grid;grid-template-columns:300px 1fr;gap:25px}
        .user-selection{background:#fff;border-radius:12px;padding:20px;box-shadow:0 4px 12px rgba(0,0,0,.08);height:fit-content}
        .section-title{font-size:18px;font-weight:600;color:#2c3e50;margin-bottom:20px;padding-bottom:10px;border-bottom:1px solid #e1e4e8}
        .search-box{position:relative;margin-bottom:20px}
        .search-box i{position:absolute;left:15px;top:50%;transform:translateY(-50%);color:#7f8c8d}
        .search-box input{width:100%;padding:12px 15px 12px 45px;border:1px solid #ddd;border-radius:8px;font-size:16px}
        .user-list{max-height:430px;overflow-y:auto}
        .user-item{display:flex;align-items:center;padding:12px;border-radius:8px;cursor:pointer;transition:background .3s;margin-bottom:8px}
        .user-item:hover{background:#f1f5f9}
        .user-item.active{background:#e3f2fd}
        .user-item img{width:40px;height:40px;border-radius:50%;object-fit:cover;margin-right:12px}
        .user-name{font-weight:500;margin-bottom:4px}
        .user-role{font-size:12px;color:#7f8c8d}
        .document-management{background:#fff;border-radius:12px;padding:25px;box-shadow:0 4px 12px rgba(0,0,0,.08)}
        .selected-user{display:flex;align-items:center;margin-bottom:25px;padding-bottom:20px;border-bottom:1px solid #e1e4e8}
        .selected-user img{width:60px;height:60px;border-radius:50%;object-fit:cover;margin-right:15px}
        .upload-section{margin-bottom:30px}
        .upload-card{background:#f8f9fa;border:2px dashed #d1d8e0;border-radius:12px;padding:25px;text-align:center;margin-bottom:20px}
        .upload-icon{font-size:40px;color:#4a6cf7;margin-bottom:15px}
        .upload-text h4{font-size:18px;margin-bottom:8px}
        .upload-text p{color:#7f8c8d}
        .file-input{display:none}

        .doc-status{display:flex;align-items:center;font-size:12px;margin-top:5px}
        .status-indicator{display:inline-flex;align-items:center;padding:3px 8px;border-radius:12px;font-weight:500;margin-right:8px}
        .status-uploaded{background:#e1f7e3;color:#27ae60}
        .status-pending{background:#fef5e7;color:#e67e22}
        .doc-action{position:absolute;top:10px;right:10px;width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;opacity:.85;transition:opacity .3s;border:none}
        .doc-action:hover{opacity:1}
        .download-action{background:#e3f2fd;color:#4a6cf7}
        .delete-action{background:#fee2e2;color:#ef4444; right:42px}
        .doc-type-btn{position:relative}

        .filter-bar{background:#fff;border-radius:12px;padding:16px 20px;margin:0 0 20px;box-shadow:0 4px 12px rgba(0,0,0,.08)}
        .filter-row{display:flex;gap:12px;align-items:center;margin-bottom:12px;flex-wrap:wrap}
        .seg{display:inline-flex;border:1px solid #e1e4e8;border-radius:10px;overflow:hidden}
        .seg-btn{padding:8px 12px;border:none;background:#fff;cursor:pointer}
        .seg-btn.active{background:#4a6cf7;color:#fff}
        .search-wide{position:relative;flex:1;min-width:220px}
        .search-wide i{position:absolute;left:12px;top:50%;transform:translateY(-50%);color:#7f8c8d}
        .search-wide input{width:100%;padding:10px 12px 10px 36px;border:1px solid #ddd;border-radius:8px;font-size:15px}
        .tabs{display:flex;gap:8px;flex-wrap:wrap}
        .tab{padding:8px 12px;border:1px solid #e1e4e8;border-radius:8px;background:#fff;cursor:pointer}
.tab.active{background:#f0f4ff;border-color:#4a6cf7;color:#4a6cf7}


        .browse-btn{background:#4a6cf7;color:#fff;border:none;padding:12px 25px;border-radius:8px;font-weight:500;cursor:pointer;transition:background .3s}
        .browse-btn:hover{background:#3b5be3}
        .document-type{display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:15px;margin-bottom:20px}
        .doc-type-btn{display:flex;align-items:center;padding:15px;border:1px solid #e1e4e8;border-radius:8px;cursor:pointer;transition:all .3s;background:#fff}
        .doc-type-btn:hover{border-color:#4a6cf7}
        .doc-type-btn.active{border-color:#4a6cf7;background:#f0f4ff}
        .doc-icon{width:40px;height:40px;border-radius:8px;background:#e3f2fd;display:flex;align-items:center;justify-content:center;margin-right:15px;color:#4a6cf7;font-size:18px}
        .upload-btn{background:#10b981;color:#fff;border:none;padding:12px 25px;border-radius:8px;font-weight:500;cursor:pointer;transition:background .3s;width:100%;font-size:16px}
        .upload-btn:hover{background:#0da271}
        .upload-btn:disabled{background:#c1c8d0;cursor:not-allowed}

        .mini-danger{background:#fee2e2;color:#b91c1c;border:none;padding:8px 12px;border-radius:8px;font-weight:500;cursor:pointer}
        .mini-danger:disabled{opacity:.5;cursor:not-allowed}
...
.expiry-panel-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:12px}
.expiry-title{font-size:16px;font-weight:700;color:#2c3e50}
.expiry-subtitle{font-size:12px;color:#7f8c8d;margin-top:4px}
.expiry-controls{display:flex;align-items:center;gap:10px;margin-bottom:12px}
.expiry-label{display:flex;align-items:center;gap:10px;font-size:13px;color:#2c3e50}
.expiry-select{padding:8px 10px;border-radius:10px;border:1px solid #ddd;background:#fff}
.expiry-body{font-size:13px}
.expiry-muted{color:#7f8c8d}
.expiry-list{display:flex;flex-direction:column;gap:10px}
.expiry-row{border:1px solid #eee;border-radius:12px;padding:10px;display:flex;justify-content:space-between;gap:10px}
.expiry-name{font-weight:700}
.expiry-email{font-size:12px;color:#7f8c8d;margin-top:2px}
.expiry-meta{font-size:12px;color:#2c3e50;margin-top:6px}
.expiry-actions{display:flex;flex-direction:column;gap:8px;align-items:flex-end}


        .toast{position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:#10b981;color:#fff;padding:12px 20px;border-radius:8px;box-shadow:0 4px 12px rgba(0,0,0,.15);z-index:1000}
        @media (max-width:900px){.main-content{grid-template-columns:1fr}.document-type{grid-template-columns:1fr}}
      `}</style>
    </main>
  );
}
