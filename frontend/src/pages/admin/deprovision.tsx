...
type DocUpcomingResp = {
  windowDays: number;
  policy: { enabled: boolean; delayAmount: number; delayUnit: string; includeAvatar: boolean };
...
function DeprovisionSettings() {
  const [section, setSection] = useState<'deprov' | 'docdel' | 'reminders'>('deprov');
  ...
  const [delayUnit, setDelayUnit] = useState<'days' | 'weeks' | 'months'>('days');
  ...
  const [docDelayUnit, setDocDelayUnit] = useState<'days'|'weeks'|'months'>('days');
...
const [loadingDocUpcoming, setLoadingDocUpcoming] = useState(false);
  ...
  setLoading(true);
  try {
    const res = await fetchWithAuth('/api/deprovision/policy', { cache: 'no-store' as RequestCache });
...
async function toggleTransfer(next: boolean) {
  ...
  if (next) {
    ...
    if (email) {
      try { await saveDeprov({ transferTargetEmail: email }); }
    ...
    try { await saveDeprov({ transferTargetEmail: null }); }
    catch { setTransferEnabled(true); }
  }
}
  ...
  async function loadUpcoming() {
    ...
    try {
      const res = await fetchWithAuth('/api/deprovision/upcoming?windowDays=7', { cache: 'no-store' as RequestCache });
...
useEffect(() => {
  ...
  try {
    const res = await fetchWithAuth('/api/deprovision/doc-cleanup/policy', { cache: 'no-store' as RequestCache });
    ...
    setDocIncludeAvatar(Boolean((p as any).includeProfileImage ?? (p as any).includeAvatar));
...
async function loadDocUpcoming(windowDays = 30) {
  ...
  try {
    const res = await fetchWithAuth(`/api/deprovision/doc-cleanup/upcoming?windowDays=${windowDays}`, {
  ...
  async function saveDeprov(patch: Partial<Pick<Policy, 'enabled' | 'delayAmount' | 'delayUnit' | 'transferTargetEmail'>>) {
    setSaving(true);
    try {
  ...
  async function toggleDeprov(next: boolean) { await saveDeprov({ enabled: next }); }
  ...
  async function runDeprovNow() {
    setRunning(true);
    try {
      const res = await fetchWithAuth('/api/deprovision/run-now', { method: 'POST', cache: 'no-store' as RequestCache });
      ...
      showToast(`Run completed • deleted ${stats.deleted}, transfers ${stats.ok}/${stats.att}, failed ${stats.fail}`);
      await Promise.all([loadPolicy(), loadUpcoming()]);
    } catch (e: any) {
  ...
  const [noticeMsg, setNoticeMsg] = useState<string>('Documents reminder sent.');
  ...
  async function loadWhitelist() {
    const r = await fetchWithAuth('/api/reminders/whitelist', { cache: 'no-store' as RequestCache });
  ...
  async function searchUsers(q: string) {
    ...
    const r = await fetchWithAuth(`/api/users/options?q=${encodeURIComponent(q)}`);
  ...
  async function removeFromWhitelist(userId: number) {
    await fetchWithAuth(`/api/reminders/whitelist/${userId}`, { method: 'DELETE' });
    loadWhitelist();
  }
  ...
  async function loadDeprovWhitelist() {
    const r = await fetchWithAuth('/api/deprovision/whitelist', { cache: 'no-store' as RequestCache });
  ...
  async function addToDeprovWhitelist(userId: number) {
    ...
    setDepQuery(''); setDepOpts([]); loadDeprovWhitelist();
  }
  async function removeFromDeprovWhitelist(userId: number) {
    await fetchWithAuth(`/api/deprovision/whitelist/${userId}`, { method: 'DELETE' });
    loadDeprovWhitelist();
  }
  ...
  async function loadRem() {
    setRemLoading(true);
    try {
      const res = await fetchWithAuth('/api/reminders/policy', { cache: 'no-store' as RequestCache });
      if (!res.ok) { setRem(null); return; }
      const data: RemPolicy = await res.json();
      setRem(data);
      setAutoPurgeEnabled(!!data.autoPurgeEnabled);
      if (Number.isFinite(data.autoPurgeDays)) {
        setRetentionMonths(Math.max(1, Math.min(6, Math.round((data.autoPurgeDays as number) / 30))));
      }
    } catch {
      setRem(null);
    } finally {
  ...
  async function loadRemHistory() {
    try {
      const res = await fetchWithAuth('/api/reminders/history?limit=20', { cache: 'no-store' as RequestCache });
      if (res.ok) {
  ...
  useEffect(() => {
  if (section === 'reminders') { void Promise.all([loadRem(), loadRemHistory(), loadWhitelist()]); }
  if (section === 'deprov')    { void Promise.all([loadPolicy(), loadUpcoming(), loadDeprovWhitelist()]); }
  if (section === 'docdel')    { void loadDocPolicy(); void loadDocUpcoming(30); }
  }, [section]);
...
async function runDocCleanupNow() {
  ...
  (typeof j?.avatarDeleted === 'number' ? ` • avatars: ${j.avatarDeleted}` : '')
);
  } catch (e:any) {
...
function onDocChangeUnit(u: 'days'|'weeks'|'months') {
  ...
  try {
    const res = await fetchWithAuth('/api/reminders/purge', {
      ...
      body: JSON.stringify({ olderThanDays: 0, applyTo: 'both', forceAll: true }),
    });
    const j = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(j?.error || `HTTP ${res.status}`);
    showToast(`Deleted • Inbox: ${j?.deleted?.inbox ?? 0} • History: ${j?.deleted?.history ?? 0}`);
    if (section === 'reminders') { void loadRemHistory(); }
  } catch (e: any) {
  ...
  async function purgeReminders(applyTo: 'inbox'|'history'|'both' = 'both') {
    try {
      const res = await fetchWithAuth('/api/reminders/purge', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function saveAutoPurgeSettings() {
    ...
    try {
      ...
      await loadRem();
    } catch (e: any) { showToast(e?.message || 'Failed to save auto-delete'); }
  }

  async function runRemNow() {
    setRemRunning(true);
    try {
      const res = await fetchWithAuth('/api/reminders/run-now', { method: 'POST' });
      const j = await res.json().catch(() => ({}));
    ...
    <div className={styles.wrapper}>
      ...
      {noticeOpen && (
        <div
          style={{
            ...
            width: 'min(94vw, 640px)', background: '#fff', borderRadius: 12, padding: 16,
            display: 'flex', gap: 12, alignItems: 'center', boxShadow: '0 12px 30px rgba(0,0,0,.18)', zIndex: 1000
          }}
        >
          <div style={{
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 600, color: '#2c3e50' }}>Document Reminder</div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <a
              className={styles.btn}
              style={{ background: '#4a6cf7', color: '#fff', border: 'none', textDecoration: 'none', display:'inline-flex', alignItems:'center' }}
      ...
      <aside className={styles.navbar}>
        <div className={styles.navHeader}>
          <div className={styles.navTitle}><span>⚙️</span><span>Cron Manager</span></div>
          <div className={styles.navDesc}>Manage all your scheduled tasks in one place</div>
        </div>
        ...
        <div className={styles.navItems}>
          ...
          >
            <div className={styles.itemTitle}><span>👩‍🎓</span><span> Deprovisioning</span></div>
            <div className={styles.itemDesc}>Manage account deprovisioning</div>
            <div className={styles.actions}>
              <button
                type="button"
                className={`${styles.btn} ${styles.btnPrimary}`}
                onClick={(e) => { e.stopPropagation(); runDeprovNow(); }}
              ...
              {enabled ? (
                <button
                  ...
                  onClick={(e) => { e.stopPropagation(); toggleDeprov(false); }}
                ...
                <button
                  ...
                  onClick={(e) => { e.stopPropagation(); toggleDeprov(true); }}
                  disabled={saving || loading}
                >
                  ✕ Disabled
                </button>
          ...
          >
            <div className={styles.itemTitle}><span>🧹</span><span>Document Deletion</span></div>
            <div className={styles.itemDesc}>Auto-delete docs after end date</div>
            <div className={styles.actions}>
              <button
                type="button"
                className={`${styles.btn} ${styles.btnPrimary}`}
                onClick={(e) => { e.stopPropagation(); runDocCleanupNow(); }}
  ...
  <button
    type="button"
    className={`${styles.btn} ${styles.btnSuccess}`}
    onClick={(e) => { e.stopPropagation(); toggleDoc(false); }}
    disabled={docSaving}
  >
    ✓ Enabled
  </button>
  ...
  <button
    type="button"
    className={`${styles.btn} ${styles.btnSecondary}`}
    onClick={(e) => { e.stopPropagation(); toggleDoc(true); }}
    disabled={docSaving}
  >
    ✕ Disabled
  </button>
          ...
          >
            <div className={styles.itemTitle}><span>📨</span><span>Document Reminders</span></div>
            <div className={styles.itemDesc}>Send & schedule missing-doc notifications</div>
            <div className={styles.actions}>
              <button
                type="button"
                className={`${styles.btn} ${styles.btnPrimary}`}
                onClick={(e) => { e.stopPropagation(); runRemNow(); }}
              ...
              {remLoading ? (
                ...
                <button
                  type="button"
                  className={`${styles.btn} ${styles.btnSuccess}`}
                  onClick={(e) => { e.stopPropagation(); saveRem({ enabled: false }); }}
                  disabled={remSaving}
                >
                  ✓ Enabled
                </button>
                ...
                <button
                  type="button"
                  className={`${styles.btn} ${styles.btnSecondary}`}
                  onClick={(e) => { e.stopPropagation(); saveRem({ enabled: true }); }}
                  disabled={remSaving}
                >
                  ✕ Disabled
                </button>
        ...
        {section === 'deprov' ? (
          <>
            <div className={styles.contentHeader}>
              ...
              <p className={styles.contentDesc}>Manage account deprovisioning settings and delayed delete options.</p>
            </div>

            <section className="bg-white rounded-xl shadow p-6">
              <h2 className="text-lg font-semibold text-[#2c3e50]">Deprovisioning Settings</h2>

              <div className={styles.statusBar}>
                <div className={styles.statusGrid}>
                  <div className={styles.statusItem}>
                    <span className={`${styles.statusIcon} ${styles.iconSky}`}><i>⏳</i></span>
                    <div>
                      <div className={styles.statusLabel}>Last run</div>
                      <div className={styles.statusValue}>{fmtDT(lastRunAt)}</div>
                    </div>
                  </div>
                  <div className={styles.statusItem}>
                    <span className={`${styles.statusIcon} ${styles.iconAmber}`}><i>🗑</i></span>
                    <div>
                      <div className={styles.statusLabel}>Deleted last run</div>
                      <div className={styles.statusValue}>{lastDeleted ?? 0} account{(lastDeleted ?? 0) === 1 ? '' : 's'}</div>
                    </div>
                  </div>
                  <div className={styles.statusItem}>
                    <span className={`${styles.statusIcon} ${styles.iconEmerald}`}><i>✓</i></span>
                    <div>
                  ...
                  <div className={styles.statusItem}>
                    <span className={`${styles.statusIcon} ${styles.iconGray}`}><i>⚙</i></span>
                    <div>
                      <div className={styles.statusLabel}>Worker</div>
                      <div className={styles.statusValue}>
                        {loading ? 'Loading…' : saving ? 'Saving…' : running ? 'Running…' : 'Idle'}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-4 rounded-lg bg-[#f8f9fa] p-4">
                <div className="text-base font-medium text-[#2c3e50] mb-3">Delayed Delete Option</div>

                <div className={styles.fieldRow}>
                  <label>
                    <div className="text-sm text-gray-600 mb-1">Delete after</div>
                    <select className={styles.select} value={delayAmount} onChange={(e) => onChangeAmount(parseInt(e.target.value, 10))} disabled={loading || saving}>
                  ...
                  <label>
                    ...
                    <select className={styles.select} value={delayUnit} onChange={(e) => onChangeUnit(e.target.value as any)} disabled={delayAmount === 0 || loading || saving}>
  ...
  <div className="text-sm text-gray-600 mb-1">Transfer Drive files</div>
  ...
  <div className={styles.inlineRow}>
    ...
    />
    <label htmlFor="transfer-toggle" className={styles.inlineRowLabel}>
  ...
  <input
    ...
    onBlur={() => {
  if (transferEnabled) {
    void saveDeprov({ transferTargetEmail: transferTargetEmail.trim() || '' });
  }
}}
    ...
    {transferEnabled
      ...
      : 'Disabled — files will NOT be transferred on deprovision.'}
            ...
            <section className={`${styles.card} ${styles.tableCard}`}>
              <h2 className="text-lg font-semibold text-[#2c3e50]">Upcoming Deprovisioning</h2>
              <p className="text-sm text-gray-600">Interns whose end date is in the next 7 days. Scheduled date includes the delay.</p>
                ...
                <table className={styles.table}>
                  ...
                  <tbody>
                    {(upcoming?.items ?? []).length === 0 ? (
                      <tr><td className={styles.td} colSpan={5}>No records in the next 7 days.</td></tr>
                    ) : (
                      upcoming!.items.map((it, idx) => (
                        <tr key={idx}>
                          <td className={styles.td}>
                            {([it.firstName, it.surname].filter(Boolean).join(' ')
                              || it.email
                              || (it.internId != null ? `ID ${it.internId}` : '-'))}
                          </td>
                          <td className={styles.td}>{it.department ?? '-'}</td>
                          <td className={styles.td}>{fmt(it.endDate)}</td>
                          <td className={styles.td}>{fmt(it.scheduledDelete)}</td>
  ...
  <div style={{ marginTop: 16 }}>
    <h2 className="text-lg font-semibold text-[#2c3e50]">Overdue Deprovisioning</h2>
    <p className="text-sm text-gray-600">
      These users’ scheduled delete date has already passed (not whitelisted).
    </p>
      ...
      <tbody>
        {upcoming!.overdue!.map((it, idx) => (
          <tr key={`od-${idx}`}>
            ...
            <section className="bg-white rounded-xl shadow p-6" style={{ marginTop: 16 }}>
              <h3 className="text-base font-semibold text-[#2c3e50]">Deprovision Whitelist</h3>
              <p className="text-sm text-gray-600">Users here will not be deleted when their end date is reached.</p>
                ...
                <div>
                  {depOpts.slice(0, 6).map((u) => (
                    <button
                      key={u.id}
                      className={styles.btn}
                      style={{ border: '1px solid #ddd', background: '#fff', marginRight: 8 }}
              ...
              <table className={styles.table} style={{ marginTop: 12 }}>
                ...
                <tbody>
                  {wlDep.length === 0 ? (
                    <tr><td className={styles.td} colSpan={4}>No whitelist entries.</td></tr>
                  ) : (
                    wlDep.map((w: any) => (
                      <tr key={w.user.id}>
                        <td className={styles.td}>{w.user.firstName} {w.user.surname} • {w.user.empId}</td>
                        <td className={styles.td}>{w.user.companyEmail}</td>
                        <td className={styles.td}>{w.user.role}</td>
                        <td className={styles.td}>
                          <button className={styles.btn} onClick={() => removeFromDeprovWhitelist(w.user.id)}>Remove</button>
                        </td>
                      </tr>
                    ))
                  )}
          ...
          <>
            <div className={styles.contentHeader}>
              ...
              <p className={styles.contentDesc}>Delete documents after a configurable grace period from their end date.</p>
            </div>

            <section className="bg-white rounded-xl shadow p-6">
  <h2 className="text-lg font-semibold text-[#2c3e50]">Deletion Settings</h2>
    ...
    <div className={styles.fieldRow}>
      <label>
        ...
        <select
          ...
          onChange={(e) => onDocChangeAmount(parseInt(e.target.value || '0', 10))}
        >
          {Array.from({ length: 31 }, (_, i) => i).map(n => (<option key={n} value={n}>{n}</option>))}
        </select>
      </label>
  ...
  try {
    ...
    disabled={docSaving}
  />
  <span className="text-sm text-gray-700">Include profile image</span>
</label>
...
>
  ...
  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
    <div>
      <h2 className="text-lg font-semibold text-[#2c3e50]">Upcoming deletions (next 30 days)</h2>
      <p className="text-sm text-gray-600">
        Based on End Date + delay ({docUpcoming?.policy.delayAmount ?? docDelayAmount} {docUpcoming?.policy.delayUnit ?? docDelayUnit})
      </p>
    </div>
  ...
  <div className="mt-4">
    {!docUpcoming || docUpcoming.items.length === 0 ? (
      <div className="text-sm text-gray-600">No upcoming document deletions.</div>
    ) : (
        ...
        <table className={styles.table}>
          ...
          <tbody>
            {docUpcoming.items.map((it) => (
              <tr key={it.internId}>
                ...
                <td className={styles.td}>{it.email ?? '—'}</td>
                <td className={styles.td}>{new Date(it.endDate).toLocaleDateString()}</td>
                <td className={styles.td}>{new Date(it.deleteAt).toLocaleDateString()}</td>
                <td className={styles.td}>{it.docsCount}</td>
                <td className={styles.td}>
                  {it.hasProfilePicture ? (it.includeAvatarByPolicy ? 'Will delete' : 'Exists (policy OFF)') : '—'}
                </td>
              </tr>
            ))}
          </tbody>
          ...
          <>
            <div className={styles.contentHeader}>
              ...
              <p className={styles.contentDesc}>Configure missing-document notifications and send them immediately.</p>
            </div>

            <section className="bg-white rounded-xl shadow p-6">
              <h2 className="text-lg font-semibold text-[#2c3e50]">Reminder Settings</h2>
              ...
              <div className={styles.statusBar}>
                <div className={styles.statusGrid}>
                  <div className={styles.statusItem}>
                    ...
                    <div>
                      <div className={styles.statusLabel}>Status</div>
                      <span className={`${styles.badge} ${(rem?.enabled ? styles.badgeActive : styles.badgeInactive)}`}>
                  ...
                  <div className={styles.statusItem}>
                    ...
                    <div>
                      ...
                      <div className={styles.statusValue}>{rem?.everyDays ?? 7} day(s)</div>
                    </div>
                  </div>
                  <div className={styles.statusItem}>
                    <span className={`${styles.statusIcon} ${styles.iconGray}`}><i>📬</i></span>
                    <div>
                      ...
                      <div className={styles.statusValue}>{rem?.emailEnabled ? 'Enabled' : 'Disabled'}</div>
                    </div>
                  </div>
                  <div className={styles.statusItem}>
                    <span className={`${styles.statusIcon} ${styles.iconAmber}`}><i>🕒</i></span>
                    <div>
                      <div className={styles.statusLabel}>Last run</div>
                      <div className={styles.statusValue}>{fmtDT(rem?.lastRunAt)}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 rounded-lg bg-[#f8f9fa] p-4">
                <div className={styles.fieldRow}>
                  ...
                  <label>
                    ...
                    <select
                      ...
                      onChange={(e) => saveRem({ everyDays: parseInt(e.target.value, 10) || 7 })}
                      disabled={remSaving}
                    >
                      {[3, 5, 7, 10, 14, 21, 28].map(n => <option key={n} value={n}>{n} day(s)</option>)}
                    </select>
                  </label>

                  <label>
                    <div className="text-sm text-gray-600 mb-1">Email notifications</div>
                    <select
                      className={styles.select}
                      value={rem?.emailEnabled ? '1' : '0'}
                      onChange={(e) => saveRem({ emailEnabled: e.target.value === '1' })}
                      disabled={remSaving}
                    >
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
                  ...
                  <button
                    type="button"
                    className={styles.btn}
                    onClick={() => { void Promise.all([loadRem(), loadRemHistory()]); showToast('Reloaded'); }}
            ...
            {/* History Retention */}
            <section className="bg-white rounded-xl shadow p-6" style={{ marginTop: 16 }}>
              <h3 className="text-base font-semibold text-[#2c3e50]">History Retention</h3>
              <p className="text-sm text-gray-600">
                Remove old reminder items (Inbox &amp; Send History) older than the selected period.
              </p>
  ...
  >
    Save Auto-delete
                ...
                <label style={{ display:'flex', alignItems:'center', gap:10 }}>
                  ...
                  />
                  <span className="text-sm text-gray-700">Auto delete (server)</span>
                </label>
              </div>

              <div className="text-xs text-gray-500 mt-2">
                “Delete older now” runs a manual cleanup. If Auto delete is enabled, the server will
                purge items older than the selected retention automatically.
              </div>
            </section>

            <section className="bg-white rounded-xl shadow p-6" style={{ marginTop: 16 }}>
              <h3 className="text-base font-semibold text-[#2c3e50]">Whitelist</h3>
              <p className="text-sm text-gray-600">Users here will not receive document reminders.</p>
                  ...
                  {wl.length === 0 ? (
                    ...
                    wl.map((w: any) => (
                      <tr key={w.user.id}>
                        ...
                        <td className={styles.td}>{w.user.role}</td>
                        <td className={styles.td}>
                          <button className={styles.btn} onClick={() => removeFromWhitelist(w.user.id)}>Remove</button>
                        </td>
                      </tr>
                    ))
                  )}
            ...
            <section className={`${styles.card} ${styles.tableCard}`}>
              <h2 className="text-lg font-semibold text-[#2c3e50]">Send History</h2>
              <p className="text-sm text-gray-600">Recent reminder runs. (This will populate as your backend starts storing history.)</p>
              ...
              <table className={styles.table}>
                ...
                <tbody>
                  {history.length === 0 ? (
                    <tr><td className={styles.td} colSpan={3}>No history yet.</td></tr>
                  ) : history.map((h, i) => (
                    <tr key={h.id ?? i}>
                      ...
                      <td className={styles.td}>
                        {h.missingBreakdown
                          ? Object.entries(h.missingBreakdown).map(([k, v]) => `${k}: ${v}`).join(' • ')
                          : '—'}
                      </td>
                    </tr>
                  ))}
