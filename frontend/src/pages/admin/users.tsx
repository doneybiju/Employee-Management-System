...
type ColId =
  | 'name' | 'companyEmail' | 'department' | 'position' | 'gender' | 'nationality' | 'status'
  | 'empID' | 'phone' | 'startDate' | 'endDate' | 'personalEmail' | 'birthdate' | 'supervisor' | 'role'
  | 'empType';

type ColCfg = { selectable: boolean; cols: ColId[]; role: 'hr'|'super_admin'; version: number; updatedAt: string };
...
const TABLE_COLS: ColId[] = ['name','companyEmail','department','position','startDate','endDate'];
...
const [editRole, setEditRole] = useState<'intern' | 'hr' | 'super_admin'>('intern');
...
// department/position
type Dept = { id:number; departmentName:string; positions: { id:number; name:string }[] };
...
const loadRows = async (whichTab: Tab = tab) => {
  ...
  useEffect(() => { if (!loading) loadRows(tab); }, [loading, tab]);
  ...
  (async () => {
    try {
      ...
      const names = Array.isArray(list) ? list.map((d: any) => d.departmentName).filter(Boolean) : [];
      setDepartments(names.sort((a: string, b: string) => a.localeCompare(b)));
    } catch {
      setDepartments([]);
    }
  })();
...
// }
  ...
  const timer = setInterval(async () => {
    try {
      const res = await fetchWithAuth('/api/users/admin/users/columns/version');
...
useEffect(() => {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r => {
      if (((r.status ?? 'inactive') as string).toLowerCase() !== activeOnly) return false;  // might uncomment 
      ...
      if (needle) {
        const hay = [
          r.name, r.companyEmail || '', r.personalEmail || '', r.employeeId || '',
          r.department || '', r.position || '', r.nationality || '',
        ].join(' ').toLowerCase();
  ...
  const resetFilters = () => { setQ(''); setDep('all'); setGen('all'); setCountry(''); };

  if (loading) return <main style={{ padding: 24 }}>Loading…</main>;
  if (!isStaff) return <main style={{ padding: 24 }}><Link href="/login">Login</Link> required.</main>;
...
const tryGsuiteDelete = async (email?: string | null): Promise<{ attempted: number; ok: boolean; error?: string }> => {
...
const deactivateActiveUser = async (r: Row) => {
  ...
  if (!res.ok) {
    if (j?.error === 'end_date_required') {
      alert('End date is required before deleting. Please set the end date and click "Save & Delete".');
...
const handleDelete = async (r: Row) => {
  if (tab === 'active') {
    ...
    if (r.internId && !r.leavingDate) {
      alert('End date is required before deleting. Set the end date, then click "Save & Delete".');
      openEdit(r, { forDelete: true });
      return;
    }

    if (!confirm('Mark internship inactive, remove Workspace account, and delete portal login?')) return;
  ...
  if (tab === 'inactive') {
  if (!r.internId) { alert('Missing intern id.'); return; }
  if (!confirm('Permanently delete this inactive intern and related data?')) return;
  try {
    const res = await fetchWithAuth(`/api/admin/users/intern/${r.internId}`, { method: 'DELETE' });
    if (!res.ok) {
      const j = await res.json().catch(() => ({}));
      throw new Error(j?.error || `HTTP ${res.status}`);
    }
...
(async () => {
  ...
  (async () => {
    ...
    const dep = rows.find(
      d => (d.departmentName || '').toLowerCase() === (r.department || '').toLowerCase()
    );
 ...
 const saveEdit = async (opts?: { thenDelete?: boolean }) => {
  ...
  if (editDeptId != null) {
    ...
    if (!dep) { alert('Invalid department'); return; }
...
const openDetails = async (r: Row) => {
  ...
  try {
    ...
    if (r.userId) {
      ...
      resp = await fetchWithAuth(`/api/admin/users/detail-by-intern/${r.internId}`);
    }
  ...
  const renderThs = () => (
    <>
      {TABLE_COLS.map(id => <th key={id}>{thLabel(id)}</th>)}
      <th>Details</th>
      <th>Actions</th>
    </>
  );
  ...
  const renderCells = (r: Row) => (
    <>
      {/* name */}
      <td style={{ whiteSpace:'nowrap', textOverflow:'ellipsis', overflow:'hidden', maxWidth:240 }}>{r.name}</td>
      {/* companyEmail */}
      <td style={{ whiteSpace:'nowrap', textOverflow:'ellipsis', overflow:'hidden', maxWidth:260 }}>{r.companyEmail || '—'}</td>
      {/* department */}
      <td>{r.department || '—'}</td>
      {/* position */}
      <td>{r.position || '—'}</td>
      ...
      <td>
        <button
          ...
          style={{ background:'#eef6ff', border:'1px solid #cfe3ff', color:'#1e62d0', padding:'6px 10px', borderRadius:8, cursor:'pointer' }}
  ...
  r.blocked ? (
    <button
      ...
      onClick={async () => { try { await unblockUser(r.userId!); await loadRows(tab); } catch { alert('Restore failed'); } }}
    ...
    <button
      title="Revoke access"
      onClick={async () => {
        if (!confirm('Revoke portal access and suspend Workspace?')) return;
        try {
    ...
    >
      🔒
          ...
          <button title="Deactivate & Delete login" onClick={() => handleDelete(r)} style={{ background:'none', border:'none', cursor:'pointer' }}>🗑️</button>
        </div>
      </td>
    </>
  );
...
const valFor = (id: ColId): string => {
  switch (id) {
    case 'name':          return (vd.firstName && vd.surname) ? `${vd.firstName} ${vd.surname}` : (r.name || '—');
    case 'companyEmail':  return vd.companyEmail ?? r.companyEmail ?? '—';
    case 'department':    return r.department || '—';
    case 'position':      return r.position || '—';
    case 'gender':        return (vd.gender ?? r.gender ?? '—').toString();
    case 'nationality':   return vd.nationality ?? r.nationality ?? '—';
    case 'status':        return (r.status || '—').toString();
    case 'empID':         return r.employeeId || '—';
    case 'phone':         return vd.phone ?? r.phone ?? '—';
    case 'startDate':     return dmy(vd.startDate ?? r.joiningDate);
    case 'endDate':       return dmy(vd.endDate ?? r.leavingDate);
    case 'personalEmail': return vd.personalEmail ?? r.personalEmail ?? '—';
    case 'birthdate':     return dmy(vd.birthdate ?? r.dob);
    case 'supervisor':    return vd.supervisor ?? r.supervisor ?? '—';
  ...
  return v ? normalizeEmpType(v) : '—';
    ...
    return ALL_COLS
      ...
      .map(c => ({ label: c.label, value: valFor(c.id) }));
  };
  ...
  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
    ...
    <Tabs tab={tab} onChange={setTab} />
      ...
      {/* Search & Filters */}
      <div style={{ background:'#fff', border:'1px solid #eee', borderRadius:10, padding:16, boxShadow:'0 4px 6px rgba(0,0,0,0.05)', marginBottom:16 }}>
        ...
        <div>
          ...
          <select value={dep} onChange={(e) => setDep(e.target.value)} style={{ width:'100%', padding:10, border:'1px solid #ddd', borderRadius:8 }}>
        ...
        <div>
          ...
          <select value={gen} onChange={(e) => setGen(e.target.value)} style={{ width:'100%', padding:10, border:'1px solid #ddd', borderRadius:8 }}>
            {['all','male','female','others'].map(v => <option key={v} value={v}>{v === 'all' ? 'All genders' : v[0].toUpperCase()+v.slice(1)}</option>)}
          </select>
        </div>
        <div>
        ...
        <div style={{ display:'flex', gap:12, alignItems:'end', justifyContent:'flex-end' }}>
          {(cfg?.selectable && isHR) && (
            <button
              onClick={() => setShowCols(true)}
              style={{ background:'#3498db', color:'#fff', border:'none', padding:'10px 16px', borderRadius:8, cursor:'pointer' }}
              title="Configure which fields HR can see in Details"
            >
              ▦ Select Columns
            </button>
          )}
          <button onClick={resetFilters} style={{ background:'#e74c3c', color:'#fff', border:'none', padding:'10px 16px', borderRadius:8, cursor:'pointer' }}>
            ⟲ Reset Filters
          </button>
        </div>
        </div>
      ...
      <div style={{ background:'#fff', border:'1px solid #eee', borderRadius:10, overflow:'hidden', boxShadow:'0 4px 6px rgba(0,0,0,0.05)' }}>
        <table width="100%" cellPadding={12} style={{ borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ textAlign:'left', background:'#f8f9fa' }}>{renderThs()}</tr>
          </thead>
          <tbody>
            {filtered.map((r, i) => {
              const key =
              r.userId != null ? `u-${r.userId}` :
              r.internId != null ? `i-${r.internId}` :
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={TABLE_COLS.length + 2} style={{ color:'#666', padding:16 }}>No users for this tab / filters.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
{isHR && cfg?.selectable && showCols && (
  <div onClick={() => setShowCols(false)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.3)', display:'grid', placeItems:'center', zIndex:1000 }}>
    <div onClick={(e) => e.stopPropagation()} style={{ width:520, maxWidth:'95vw', maxHeight:'80vh', overflow:'hidden', background:'#fff', borderRadius:12, padding:16 }}>
      <h2 style={{ margin:'0 0 12px' }}>Select Columns (HR)</h2>
      <div style={{ overflow:'auto', maxHeight:380, border:'1px solid #eee', borderRadius:8, padding:12 }}>
        {ALL_COLS.map(c => (
          <label key={c.id} style={{ display:'block', padding:'8px 6px' }}>
            <input
              type="checkbox"
              checked={localCols.has(c.id)}
              onChange={e => {
                setLocalCols(s => { const n = new Set(s); e.target.checked ? n.add(c.id) : n.delete(c.id); return n; });
              }}
              style={{ marginRight:8 }}
            />
            {c.label}
      ...
      <div style={{ display:'flex', justifyContent:'flex-end', gap:10, marginTop:12 }}>
        <button onClick={() => setShowCols(false)} style={{ padding:'8px 12px', border:'1px solid #ddd', borderRadius:8, background:'#fff' }}>Cancel</button>
        <button onClick={saveCols} style={{ padding:'8px 12px', border:'none', borderRadius:8, background:'#1e90ff', color:'#fff' }}>Save</button>
      </div>
      <p style={{ marginTop:10, color:'#6b7280', fontSize:12 }}>
        Table always shows 6 basic fields. This controls extra fields in the Details modal for HR.
      </p>
    </div>
  </div>
)}
      ...
      {edit && (
        <div
          ...
          style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.3)', display:'grid', placeItems:'center', zIndex:1000 }}
        >
          <div onClick={(e) => e.stopPropagation()} style={{ width:520, maxWidth:'95vw', background:'#fff', borderRadius:12, padding:16 }}>
  ...
  {editingName ? (
    <input
      ...
      style={{ fontSize:18, fontWeight:600, border:'1px solid #ddd', borderRadius:6, padding:'4px 8px' }}
    />
  ) : (
    <span
...
<div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
  <label>
    ...
    <select
      value={editDeptId ?? ''}
      onChange={(e) => { const v = e.target.value ? Number(e.target.value) : null; setEditDeptId(v); setEditPosId(null); }}
  ...
  <label>
    ...
    <select
      value={editPosId ?? ''}
      onChange={(e) => setEditPosId(e.target.value ? Number(e.target.value) : null)}
      style={{ width:'100%', padding:10, border:'1px solid #ddd', borderRadius:8 }}
      disabled={editDeptId == null}
    >
      <option value="">—</option>
      {(depts.find(d => d.id === editDeptId)?.positions || []).map(p => (
  ...
  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
    <label>
      ...
      <input type="date" value={editStart} onChange={(e) => setEditStart(e.target.value)} style={{ width:'100%', padding:10, border:'1px solid #ddd', borderRadius:8 }} />
    </label>
    <label>
      <div style={{ fontSize:13, color:'#6b7280' }}>End date</div>
      <input type="date" value={editEnd} onChange={(e) => setEditEnd(e.target.value)} style={{ width:'100%', padding:10, border:'1px solid #ddd', borderRadius:8 }} />
    </label>
  </div>
  ...
  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
    <label>
      ...
      <input value={editPhone} onChange={(e) => setEditPhone(e.target.value)} style={{ width:'100%', padding:10, border:'1px solid #ddd', borderRadius:8 }} />
    </label>
    <label>
      <div style={{ fontSize:13, color:'#6b7280' }}>Personal email</div>
      <input type="email" value={editPersonalEmail} onChange={(e) => setEditPersonalEmail(e.target.value)} style={{ width:'100%', padding:10, border:'1px solid #ddd', borderRadius:8 }} />
    </label>
  </div>
  ...
  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12 }}>
    <label>
      ...
      <input value={editNationality} onChange={(e) => setEditNationality(e.target.value)} style={{ width:'100%', padding:10, border:'1px solid #ddd', borderRadius:8 }} />
    </label>
    <label>
      <div style={{ fontSize:13, color:'#6b7280' }}>Gender</div>
      <select value={editGender} onChange={(e) => setEditGender(e.target.value)} style={{ width:'100%', padding:10, border:'1px solid #ddd', borderRadius:8 }}>
        <option value="">—</option>
    ...
    <label>
      ...
      <input type="date" value={editBirthdate} onChange={(e) => setEditBirthdate(e.target.value)} style={{ width:'100%', padding:10, border:'1px solid #ddd', borderRadius:8 }} />
    </label>
  </div>
  ...
  {isSA && edit?.userId && (
    <label>
      ...
      <select value={editRole} onChange={(e) => setEditRole(e.target.value as any)} style={{ width:'100%', padding:10, border:'1px solid #ddd', borderRadius:8 }}>
...
<label style={{ display:'block', marginTop:12 }}>
  ...
  <button
    onClick={() => { setEdit(null); setPendingDelete(null); }}
  ...
  {pendingDelete && (
    <button
      ...
      style={{ padding:'8px 12px', border:'none', borderRadius:8, background:'#ef4444', color:'#fff' }}
    >
      Save & Delete
    </button>
  )}
  ...
  <button
    ...
    style={{ padding:'8px 12px', border:'none', borderRadius:8, background:'#10b981', color:'#fff' }}
  >
    Save
  </button>
</div>
            ...
            {!edit?.internId && (
              <p style={{ marginTop:8, color:'#b45309' }}>
                (This looks like a staff account. Edit for staff isn’t wired yet.)
              </p>
            )}
          </div>
        </div>
      ...
      {view && (
        <div
          ...
          style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.35)', display:'grid', placeItems:'center', zIndex:1100 }}
        >
          <div onClick={(e) => e.stopPropagation()} style={{ width:640, maxWidth:'95vw', maxHeight:'85vh', overflow:'auto', background:'#fff', borderRadius:12, padding:18, boxShadow:'0 10px 30px rgba(0,0,0,.15)' }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8 }}>
              <h3 style={{ margin:0 }}>Details: {view.name}</h3>
              <button onClick={() => setView(null)} style={{ border:'1px solid #ddd', background:'#fff', borderRadius:8, padding:'6px 10px', cursor:'pointer' }}>Close</button>
            </div>
  ...
  const vd = viewDetail || {};
  const sos = (vd.sos ?? null) as { relation: string | null; phone: string | null } | null;

  return (
    <>
      <div style={{ display:'grid', gridTemplateColumns:'180px 1fr', gap:'10px 16px' }}>
        {items.map((it, idx) => (
          <FragmentRow key={idx} label={it.label} value={it.value} />
        ))}
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'180px 1fr', gap:'10px 16px' }}>
        ...
        <FragmentRow label="SOS Relation" value={sos?.relation ?? '—'} />
      </div>
    </>
  );
})()}
