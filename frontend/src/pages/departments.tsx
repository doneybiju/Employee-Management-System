...
export default function DepartmentsPage() {
  ...
  const setPosVal = (i: number, v: string) => setPos((p) => p.map((x, idx) => (idx === i ? v : x)));
  const rmPosInput = (i: number) => setPos((p) => p.filter((_, idx) => idx !== i));
  ...
  const delDept = async (id: number) => {
    if (!confirm('Delete this department? Positions will be removed if unused.')) return;
    try {
  ...
  const delPos = async (id: number) => {
    ...
    try {
      await fetchWithAuth(`/api/departments/positions/${id}`, { method: 'DELETE' });
      await load();
    } catch (e: any) {
      alert(e.message || 'Delete failed');
    }
  ...
  if (!user) return <div><Link href="/login">Login</Link> required.</div>;
  ...
  return (
    <main style={{ maxWidth: 1000, margin: '2rem auto', padding: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        ...
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search department…" style={{ padding: 8 }} />
      </div>

      {/* Create department */}
      <section style={{ border: '2px solid #d1d5db', padding: 20, borderRadius: 12, marginTop: 12, background: '#fff' }}>
        <h3>Create Department</h3>
        <form onSubmit={submitDept} style={{ display: 'grid', gap: 8, maxWidth: 600 }}>
          <label>
            <div style={{ fontSize: 12, color: '#666' }}>Department name</div>
            <input value={deptName} onChange={(e) => setDeptName(e.target.value)} />
          </label>
          <div>
            <div style={{ fontSize: 12, color: '#666', marginBottom: 4 }}>Positions</div>
            {pos.map((p, i) => (
              <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 6 }}>
                <input value={p} onChange={(e) => setPosVal(i, e.target.value)} style={{ flex: 1 }} />
                {pos.length > 1 && <button type="button" onClick={() => rmPosInput(i)}>−</button>}
              </div>
            ))}
            <button type="button" onClick={addPosInput}>+ Add position</button>
          </div>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            {d.positions.map((p) => (
              ...
              >
                ...
                <button type="button" title="Rename" onClick={() => renamePos(p)} style={{ width: 22, height: 22, lineHeight: '20px', textAlign: 'center', border: '1px solid #cbd5e1', borderRadius: 6, background: '#fff', cursor: 'pointer' }}>✎</button>
                <button type="button" title="Delete" onClick={() => delPos(p.id)} style={{ width: 22, height: 22, lineHeight: '20px', textAlign: 'center', border: '1px solid #fecaca', borderRadius: 6, background: '#fff', color: '#b91c1c', cursor: 'pointer' }}>×</button>
              </span>
            ))}
            <button type="button" onClick={() => addPosition(d.id)}>+ add</button>
          </div>
        </td>
        <td style={{ whiteSpace: 'nowrap' }}>
          <button onClick={() => renameDept(d)}>Rename</button>{' '}
          <button onClick={() => delDept(d.id)} style={{ color: '#b00' }}>Delete</button>
        </td>
      </tr>
      <tr aria-hidden>
        <td colSpan={3}>
          <div style={{ height: 2, background: '#d1d5db', margin: '18px 0' }} />
        </td>
      </tr>
    </React.Fragment>
  ))}
