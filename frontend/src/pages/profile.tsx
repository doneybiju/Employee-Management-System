...
useEffect(() => {
  ...
  (async () => {
    if (!data?.avatarUrl) { setAvatarSrc(DEFAULT_AVATAR); return; }
...
async function uploadAvatarBlob(blob: Blob, fileName = 'avatar.webp', mime = 'image/webp') {
  try {
    setUpAvatar(true);
    ...
    const upRes = await fetchWithAuth('/api/uploads/drive/avatar', { method: 'POST', body: fd as any });
  ...
  async function saveSOS() {
    try {
      ...
      await fetchWithAuth('/api/profile/sos', {
        ...
        body: JSON.stringify({ relativePhoneNumber: sosPhone || null, relationWithIntern: sosRelation || null }),
      });
      setData(p => (p ? { ...p, sos: { relativePhoneNumber: sosPhone || null, relationWithIntern: sosRelation || null } } : p));
      showToast('SOS information saved', 'success');
    } catch (e: any) { alert(e?.message || 'Failed to save SOS'); } finally { setSavingSOS(false); }
  }
  ...
  async function saveSocial() {
    try {
      ...
      setData(p =>
  p ? { ...p, documents: { ...normDocs(p.documents), linkedin: linkedin || null } } : p
);
showToast('Social information saved', 'success');


    } catch (e: any) { alert(e?.message || 'Failed to save Social'); } finally { setSavingSocial(false); }
  }
...
const nameBase = `${data?.firstName || 'user'}_${data?.surname || ''}`.trim();
    ...
    <main className="profile-container">
      ...
      <div className="profile-header">
        ...
        <Link href="/change-password" className="change-password-btn">
          <i className="fas fa-lock icon-blue" /> <span className="ml-8">Change password</span>
        </Link>
      </div>
  ...
  alt="Profile Avatar"
  className="avatar"
  onError={(e) => { const el = e.currentTarget; if (el.src !== DEFAULT_AVATAR) el.src = DEFAULT_AVATAR; }}
/>
          </div>

          <div>
            <h2 className="user-name">{data ? `${data.firstName} ${data.surname}` : '—'}</h2>
  ...
  <label htmlFor="avatarFile" className="upload-btn" style={{ cursor: 'pointer', display: 'inline-flex', alignItems: 'center' }}>
    {upAvatar ? <i className="fas fa-spinner fa-spin" /> : <i className="fas fa-upload" />}
    <span className="ml-8">{data?.avatarUrl ? 'Replace photo' : 'Upload photo'}</span>
  </label>
  ...
  <AvatarCropper
    ...
    onCropped={(blob) => uploadAvatarBlob(blob, 'avatar.webp', blob.type || 'image/webp')}
    circle
    size={512}
  />
)}
        ...
        <section className="profile-content">
            ...
            {canEdit ? (
              <>
                <Field label="Relative Phone Number">
                  <input value={sosPhone} onChange={(e) => setSosPhone(e.target.value)} placeholder="+123 456 789" />
                </Field>
                <Field label="Relation with Intern">
                  <select value={sosRelation} onChange={(e) => setSosRelation(e.target.value)}>
                    <option value="">—</option>
                    {['mother','father','sister','brother','grandfather','grandmother','spouse','uncle','aunt','friend']
                      .map(v => <option key={v} value={v}>{cap(v)}</option>)}
                  </select>
                </Field>
                <button className="save-btn" onClick={saveSOS} disabled={savingSOS}>
                  <i className={`fas ${savingSOS ? 'fa-spinner fa-spin' : 'fa-save'}`} />
                  <span className="ml-8">{savingSOS ? 'Saving…' : 'Save SOS Information'}</span>
                </button>
              </>
            ) : (
              <>
                <Info k="Relative Phone Number" v={data?.sos?.relativePhoneNumber || null} />
                <Info k="Relation with Intern" v={data?.sos?.relationWithIntern || null} />
              </>
            )}
          </Card>
          ...
          <Card icon="fa-share-alt" title="Social Information">
            {canEdit ? (
              <>
                ...
                <button className="save-btn" onClick={saveSocial} disabled={savingSocial}>
                  <i className={`fas ${savingSocial ? 'fa-spinner fa-spin' : 'fa-save'}`} />
                  <span className="ml-8">{savingSocial ? 'Saving…' : 'Save Social Information'}</span>
                </button>
              </>
            ) : (
          ...
          <Card icon="fa-file-alt" title="Documents">
            <div className="doc-grid">
              <DocTile label="Acceptance Letter" value={data?.documents?.acceptanceLetter}
  kind="acceptance_letter" busy={false} onPick={() => {}}
  canEdit={false} nameBase={nameBase} />

<DocTile label={agreementLabel(data?.empType)} value={data?.documents?.learningAgreement}
  kind="learning_agreement" busy={false} onPick={() => {}}
  canEdit={false} nameBase={nameBase} />
...
<DocTile
  ...
  extra={
    data?.documents?.passportExpiryDate
      ? <>Expiry: <b>{String(data?.documents?.passportExpiryDate).slice(0, 10)}</b></>
      : null
  }
/>


<DocTile label="CV" value={data?.documents?.cv}
  kind="cv" busy={false} onPick={() => {}}
  canEdit={false} nameBase={nameBase} />
            </div>
          </Card>
        </section>
      ...
      {toast && (
  <div className={`toast ${toast.kind === 'success' ? 'toast-success' : 'toast-error'}`} role="status" aria-live="polite">
    <i className={`fas ${toast.kind === 'success' ? 'fa-check-circle' : 'fa-exclamation-triangle'}`} />
...
function Info({ k, v }: { k: string; v?: string | null }) {
  return (
    <div className="info-item">
      ...
      <div className="info-value">{v && String(v).trim() ? v : <span className="empty">Not set</span>}</div>
    </div>
  );
}
...
function DocTile({
  label, value, kind, busy, onPick, canEdit, nameBase, extra,
}: {
  label: string;
    ...
    <div className="doc-tile">
      ...
      <div className="doc-mid">
        ...
        <div className="doc-file">
          {proxied ? (
            <span className="ok" style={{ color: '#16a34a', fontWeight: 600 }}>Uploaded</span>
          ) : (
    ...
    {extra}
        ...
        {proxied && (
          <div className="doc-actions" style={{ marginTop: 6 }}>
            ...
            >
              <i className="fas fa-download" /> <span className="ml-6">Download</span>
            </button>
          </div>
        )}
      </div>
        ...
        {canEdit && (
          <>
            <label htmlFor={id} className="upload-btn">
              {busy ? <i className="fas fa-spinner fa-spin" /> : <i className="fas fa-upload" />}
            ...
            <input
              ...
              type="file"
              style={{ display: 'none' }}
              accept={kind === 'passport_id' ? 'image/*,application/pdf' : 'application/pdf,image/*'}
...
function basename(p: string) { try { return decodeURIComponent(p.split('/').pop() || p); } catch { return p; } }
