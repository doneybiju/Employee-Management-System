...
async function safeErr(res: Response) {
  try {
    ...
    return (j as any)?.error as string | undefined;
  } catch {
    return undefined;
  }
}
...
export async function blockUser(userId: number) {
  const r = await fetchWithAuth(`/api/users/${userId}/block`, {
    method: 'PATCH',
  });
...
export async function unblockUser(userId: number) {
  const r = await fetchWithAuth(`/api/users/${userId}/unblock`, {
    method: 'PATCH',
  });
